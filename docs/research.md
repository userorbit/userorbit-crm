...
- Partial: email exists through outbound SMTP, stored events, sender rotation, first-party open/click tracking, provider inbound parse webhooks, parsed inbound reply ingestion, unsubscribe handling, and manual reply handling but lacks provider-native IMAP/Gmail/Outlook mailbox sync.
- Missing: deeper native third-party integrations beyond Slack/Teams/Discord notifications, Google/Microsoft calendar sync, Twilio-compatible calling/messaging, and HubSpot/Pipedrive imports.
