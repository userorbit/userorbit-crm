...
- Present: accounts, account custom fields, contacts, account detail timelines, contact detail timelines, saved account views, opportunities, pipeline board, tasks, email templates, sequences, manual sends, scheduled sequence processing, mailbox warmup, REST API, agent command API, landing page, token-gated UI, users, team memberships, teams, workspaces, workspace-scoped agent tokens, token revocation, audit logs, account CSV export, and reporting for pipeline health/activity/tasks/sequences/stalled opportunities.
